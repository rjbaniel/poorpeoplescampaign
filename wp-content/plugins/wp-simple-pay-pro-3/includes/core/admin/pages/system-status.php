	...
	private static function stripe_tls_check( $for_export ) {
		...
		try {
			...
			if ( $for_export ) {
				...
				return '<mark class="error">' . $retval . ' ' . sprintf( __( '<a href="%s">Please read this</a> for more information.', 'simple-pay' ), 'https://stripe.com/blog/upgrading-tls' ) . '</mark>';
			}
		}
	}
			...
			<div id="simpay-settings">
				<div id="simpay-settings-content">
					...
					$sections['simpay']['version'] = array(
						'label'        => sprintf( __( '%s Version', 'simple-pay' ), SIMPLE_PAY_PLUGIN_NAME ),
					...
					$sections['server'] = array(
						...
						'curl_init'           => array(
							'label'         => 'cURL',
							'result'        => ! empty( $curl_info ) ? $curl_info['version'] . ', ' . $curl_info['ssl_version'] : __( 'No version found.', 'simple-pay' ),
